...
const Menu = ({ isMobile, closeMenu, ...rest }) => {
  const staticCategories = useSelector(
    (state) => state.general.staticCategories
  );
  const genres = useSelector((state) => state.general.genres);
  const selectedMenuItemName = useSelector(
    (state) => state.general.selectedMenuItemName
  );
